function checkAccount(){
    ...
    if (!account.match(/[A-Za-z0-9]{4,11}/)){
		$('#acc-error').html('Аккаунт может содержать только буквы и цифры. От 4 до 11 знаков');
		$('#acc-error').show();
	}
}
...
function checkPassword(){
    ...
    if ( pass.length < 8 ) {
        $('#pass-error').html('Минимальный размер пароля 8 символов');
        $('#pass-error').show();
    }
}
...
function checkConfirmPassword(){
    ...
    if ( pass !=  confirm_pass ) {
        $('#cpass-error').html('Пароли должны совпадать');
        $('#cpass-error').show();
    }
}
...
(function($){
        $.fn.shortPass = 'Минимальный размер пароля 8 символов';
        $.fn.badPass = 'Слабая надежность';
        $.fn.goodPass = 'Хорошая надежность';
        $.fn.averagePass = 'Средняя надежность';
        $.fn.strongPass = 'Высокая надежность ';
        $.fn.samePassword = 'Аккаунт и пароль одинаковые';
        $.fn.resultStyle = "";
